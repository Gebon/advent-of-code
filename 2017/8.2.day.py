...
regex = re.compile(r'(?P<registry>\w+)\s+(?P<op>inc|dec)\s+(?P<amount>-?\d+)\s+if\s+(?P<condition_registry>\w+)\s+(?P<condition>[^\s]+)\s+(?P<condition_value>-?\d+)')
...
for instruction in instructions:
    ...
    if eval(('%d ' + match['condition'] + ' %s') % (registries[match['condition_registry']], match['condition_value'])):
        if match['op'] == 'inc':
            registries[match['registry']] += int(match['amount'])
        else:
            registries[match['registry']] -= int(match['amount'])
        maximum = max(maximum, registries[match['registry']])

print(max(registries.values()))
print(maximum)