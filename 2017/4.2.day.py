def is_valid(passphrase):
    for i, word in enumerate(passphrase):
        for j, word2 in enumerate(passphrase):
            if i != j and sorted(word) == sorted(word2):
                return False

    return True

print(sum(map(is_valid, map(lambda passphrase: passphrase.split(' '), '''nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
