...
class Vector:
    def __init__(self, x, y, z, modifier=None):
        self.x = x
        self.y = y
        self.z = z
    ...
    def length(self):
        return sum(abs(num) for num in [self.x, self.y, self.z])

    def tick(self):
        if not self.modifier:
            return
        self.modifier.tick()
        self.move(self.modifier)
    ...
    def is_stable(self):
        if not self.modifier:
            return True
        is_self_stable = (self.x * self.modifier.x > 0 or self.modifier.x == 0) and (self.y * self.modifier.y > 0 or self.modifier.y == 0) and (self.z * self.modifier.z > 0 or self.modifier.z == 0)
        return is_self_stable and self.modifier.is_stable()
    ...
    @staticmethod
    def parse(string):
        return Vector(*[int(num) for num in string.split(',')])


VECTOR_EXTRACTOR = re.compile(r'\w=<(?P<vector_repr>[^>]+)')

for i, row in enumerate('''p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
...
p=<-2954,-985,167>, v=<-418,-140,23>, a=<29,7,-3>'''.split('\n')):
    p, v, a = row.split(', ')
    vectors.append(
        Vector.parse(VECTOR_EXTRACTOR.match(p)['vector_repr']) \
            .set_modifier(
                Vector.parse(VECTOR_EXTRACTOR.match(v)['vector_repr']) \
                    .set_modifier(Vector.parse(VECTOR_EXTRACTOR.match(a)['vector_repr']))
            )
    )

print(vectors[0].is_stable())

while not all(vector.is_stable() for vector in vectors):
    for vector in vectors:
        vector.tick()
    points = {}
    
    for i in range(len(vectors)):
        if vectors[i].as_point() in points:
            points.remove(vectors[i].as_point())



min_index = min_length = 2**20
