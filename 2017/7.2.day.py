...
regex = re.compile(r'(?P<name>\w+)\s\((?P<weight>\d+)\)(?:\s->\s(?P<dependants>(?:\w+(?:,\s)?)+))?')
...
for row in input:
    ...
    weights[match['name']] = int(match['weight'])

    for dependency in (x for x in (match['dependants'] or '').split(', ') if x != ''):
        graph[match['name']].append(dependency)
        has_parent.add(dependency)

root = list(graph.keys() - has_parent)[0]

def get_full_weight(node):
    return sum((get_full_weight(child) for child in graph[node])) + weights[node]

def find_diff(node):
    children_weights = [get_full_weight(child) for child in graph[node]]
    print(children_weights)
    if len(set((get_full_weight(child) for child in graph[node]))) <= 1:
        return 0
    for child in graph[node]:
        diff = find_diff(child)
        if diff:
            return diff
    distinct_weights = set(children_weights)
    for distinct_weight in distinct_weights:
        if sum(distinct_weight == x for x in children_weights) == 1:
            return list(distinct_weights - set([distinct_weight]))[0]

print(find_diff(root))